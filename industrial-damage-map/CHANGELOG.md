...
Application only. **No dataset change** — facility, incident and source counts,
confidence scores and verification statuses are all untouched.

### Fixed — the map was not reliably a map

- **Bundled a fallback basemap.** OSM raster tiles are fetched from a third
  party over the public internet; on a restricted network they never arrive and
  the map degraded to markers floating on an empty rectangle. A 178 KB Natural
  Earth coastline (public domain) now ships in `public/basemap/` and is drawn
  underneath the tiles, so the geography is always present. Regenerated by
  `scripts/build-basemap.mjs`, which fails the build rather than silently
  shipping Crimea inside the Russian land polygon.
- **Cluster counts were never drawn.** The map style has no `glyphs` URL, so a
  MapLibre symbol layer using `text-field` renders nothing at all — every
  cluster was an unlabelled disc. Counts are now canvas-rendered numeral icons,
  which need no font server and no network.
- **The legend hid records.** `fitBounds` framed the data without accounting for
  the legend floating over the bottom-left, so the Black Sea and Crimea records
  sat underneath the panel on first load. Padding is now measured from the
  overlay.
- Cleared two Next.js configuration warnings: `serverComponentsExternalPackages`
  moved to `serverExternalPackages`, and `outputFileTracingRoot` is pinned to
  this directory so the standalone bundle stops tracing the parent repository.

### Added

- Four end-to-end tests, all with `tile.openstreetmap.org` blocked: markers
  paint, the coastline paints, clusters are labelled, and the basemap asset is
  served same-origin with Crimea separated. 26 Playwright tests in total.
- `scripts/screenshots.mjs` (`npm run screenshots`) regenerates `screenshots/`
  reproducibly, with tile requests blocked on purpose — a screenshot that only
  looks right because a third party happened to be reachable is not documenting
  what most readers will see.
- `data-testid="view-map"` on the map/table toggle.
